...
#steps/train_sgmm2_group.sh --cmd "$train_cmd" \
 steps/train_sgmm2.sh --cmd "$train_cmd" --nj 80 \
  ...
  $train_cmd $graph_dir/mkgraph.log \
    utils/mkgraph.sh data/lang_test exp/sgmm2_5 $graph_dir
  steps/decode_sgmm2.sh --nj 80 \
...
steps/align_sgmm2.sh --nj 80 --cmd "$train_cmd" \
...
#steps/make_denlats_sgmm2.sh --nj 80 --sub-split 30 --num-threads 6 \
steps/make_denlats_sgmm2.sh --nj 80 --sub-split 30 --num-threads 1 \
