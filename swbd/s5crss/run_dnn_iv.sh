...
run_dnn_iv_extract(){
   ...
   sid/extract_ivectors_dnn.sh --cmd "$train_cmd" --nj 80 \
        exp/extractor_dnn exp/dnn5b_pretrain-dbn_dnn data/eval2000 data-fbank/eval2000 data/eval2000.dnn-iv || exit 1;

   sid/extract_ivectors_dnn.sh --cmd "$train_cmd" --nj 80 \
        exp/extractor_dnn exp/exp/dnn5b_pretrain-dbn_dnn data/train_nodup data-fbank/train_nodup data/train_nodup.dnn-iv || exit 1;
