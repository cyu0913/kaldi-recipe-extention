...
ivector_dim=25 # dimension of the extracted i-vector
...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj_full $dir/log/gselect.JOB.log \
    gmm-gselect --n=20 $dir/final.dubm "$feats" ark:- \| \
    fgmm-global-gselect-to-post-mat --min-post=$min_post $dir/final.ubm "$feats" \
      ark,s,cs:-  ark,t:$dir/post.JOB $dir/log ark,t:$dir/log/post.table.JOB || exit 1;
else
  ...
  $cmd JOB=1:$nj_full $dir/log/copyhtk.JOB.log \
       copy-feats-to-htk --output-dir=$dir --output-ext=fea  --sample-period=100000 ark:$dir/log/post.table.JOB || exit 1;
