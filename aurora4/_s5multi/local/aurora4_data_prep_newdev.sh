...
cat $AURORA/lists/training_multicondition_16k.list \
  | $local/aurora2flist.pl $AURORA | grep -v 01yc021d | sort -u > train_si84_multi.flist

#Dev Set (According to database structure of second Aurora corpus in NTT)
for x in clean_wv1 clean_wv2 car_wv1 car_wv2 babble_wv1 babble_wv2 airport_wv1 airport_wv2 street_wv1 street_wv2 train_wv1 train_wv2 restaurant_wv1 restaurant_wv2; do
    ls -d $AURORA_DEV/dev_set_${x}_swap/*16k/*.* | sort -u > dev_1206_${x}.flist
done
...
for x in clean_wv1 clean_wv2 car_wv1 car_wv2 babble_wv1 babble_wv2 airport_wv1 airport_wv2 street_wv1 street_wv2 train_wv1 train_wv2 restaurant_wv1 restaurant_wv2; do
  ...
  ' $condition="$ARGV[0]";
    if ($condition eq "clean_wv1") {$suffix=0;}
    elsif ($condition eq "car_wv1") {$suffix=1;} 
    elsif ($condition eq "babble_wv1") {$suffix=2;} 
    elsif ($condition eq "restaurant_wv1") {$suffix=3;} 
    elsif ($condition eq "street_wv1") {$suffix=4;} 
    elsif ($condition eq "airport_wv1") {$suffix=5;} 
    elsif ($condition eq "train_wv1") {$suffix=6;} 
    elsif ($condition eq "clean_wv2") {$suffix=7;} 
    elsif ($condition eq "car_wv2") {$suffix=8;} 
    elsif ($condition eq "babble_wv2") {$suffix=9;} 
    elsif ($condition eq "restaurant_wv2") {$suffix=a;} 
    elsif ($condition eq "street_wv2") {$suffix=b;} 
    elsif ($condition eq "airport_wv2") {$suffix=c;} 
    elsif ($condition eq "train_wv2") {$suffix=d;} 
...
cat dev_1206_*_wv1.trans1  dev_1206_*_wv2.trans1 | sort -k1 > dev_1206.trans1
...
noiseword="<NOISE>";
for x in train_si84_clean train_si84_multi test_eval92 test_0166 dev_1206; do
...
for x in train_si84_clean train_si84_multi test_eval92 test_0166 dev_1206; do
    awk '{printf("%s /data/work2/chengzhu/work/Toolhouse/sox-14.4.1/local/bin/sox -B -r 16k -e signed -b 16 -c 1 -t raw %s -t wav - |\n", $1, $2);}' < ${x}_sph.scp \
    > ${x}_wav.scp
done
