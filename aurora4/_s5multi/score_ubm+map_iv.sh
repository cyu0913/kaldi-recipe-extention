...
mkdir -p exp/trials;
mkdir -p exp/score.ubm+map; rm -rf score.ubm+map/*
...
run_cds_score(){
    ...
    echo "CDS EER : `compute-eer exp/score.ubm+map/cds.score.key 2> exp/score.ubm+map/cds_EER`"
}
run_cds_score

run_lda_plda(){
    mkdir -p exp/score.ubm+map/ivector_plda; rm -rf exp/score.ubm+map/ivector_plda/*

    ivector-compute-lda --dim=50 --total-covariance-factor=0.1 \
        'ark:ivector-normalize-length scp:data/train_si84_multi.ubm+map-iv/ivector.scp ark:- |' \
        ark:data/train_si84_multi/utt2spk \
        exp/score.ubm+map/ivector_plda/lda_transform.mat 2> exp/score.ubm+map/ivector_plda/lda.log

    ivector-compute-plda ark:data/train_si84_multi/spk2utt \
          'ark:ivector-transform exp/score.ubm+map/ivector_plda/lda_transform.mat scp:data/train_si84_multi.ubm+map-iv/ivector.scp ark:- | ivector-normalize-length ark:-  ark:- |' \
            exp/score.ubm+map/ivector_plda/plda 2>exp/score.ubm+map/ivector_plda/plda.log
    ...
    paste exp/score.ubm+map/ivector_plda/plda.score $trials_key > exp/score.ubm+map/ivector_plda/plda.score.key
    echo "PLDA EER : `compute-eer exp/score.ubm+map/ivector_plda/plda.score.key 2> exp/score.ubm+map/ivector_plda/plda_EER`"
}
run_lda_plda
