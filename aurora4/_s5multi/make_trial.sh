...
make_trial(){
         ...
         for i in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | awk 'BEGIN {srand()} !/^$/ { if (rand() <= .2) print $0}'`;do
               for j in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | awk 'BEGIN {srand()} !/^$/ { if (rand() <= .2) print $0}'`;do
...
make_trial_clean(){
         ...
         # Check similarity of all i-vectors belong to two desired speakers. Code below compare speaker '01jo' vs '40po'   
         for i in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*0$" |  awk 'BEGIN {srand()} !/^$/ { if (rand() <= .4) print $0}'`;do
               for j in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*0$" | awk 'BEGIN {srand()} !/^$/ { if (rand() <= .4) print $0}'`;do
                   si=`grep $i data/test_eval92/utt2spk | awk '{print $2}'`
                   sj=`grep $j data/test_eval92/utt2spk | awk '{print $2}'`
...
make_trial_noise(){
         ...
         # Check similarity of all i-vectors belong to two desired speakers. Code below compare speaker '01jo' vs '40po'   
         for i in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*[1-6]$" |  awk 'BEGIN {srand()} !/^$/ { if (rand() <= .1) print $0}'`;do
               for j in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*[1-6]$" |  awk 'BEGIN {srand()} !/^$/ { if (rand() <= .1) print $0}'`;do
                   si=`grep $i data/test_eval92/utt2spk | awk '{print $2}'`
                   sj=`grep $j data/test_eval92/utt2spk | awk '{print $2}'`
...
make_trial_channel_noise(){
         ...
         # Check similarity of all i-vectors belong to two desired speakers. Code below compare speaker '01jo' vs '40po'   
         for i in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*[8-d]$" |  awk 'BEGIN {srand()} !/^$/ { if (rand() <= .05) print $0}'`;do
               for j in `cat data/test_eval92.iv/ivector.scp | awk '{print $1}' | grep ".*[8-d]$" |  awk 'BEGIN {srand()} !/^$/ { if (rand() <= .05) print $0}'`;do
                   si=`grep $i data/test_eval92/utt2spk | awk '{print $2}'`
                   sj=`grep $j data/test_eval92/utt2spk | awk '{print $2}'`
