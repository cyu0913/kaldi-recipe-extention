...
if [ "$copy_feats" == "true" ]; then
  ...
  echo $hname
  mkdir -p /home/cxy110530/work/kaldi_tmp_trash/$hname
  tmpdir=$(mktemp -d /home/cxy110530/work/kaldi_tmp_trash/$hname/tmp_XXXX); mv $dir/train.scp{,_non_local}; mv $dir/cv.scp{,_non_local};rm -rf $tmpdir; mkdir -p $tmpdir;
...
# read the features,
feats_tr="ark:copy-feats scp:$dir/train.scp ark:- | apply-cmvn --norm-vars='true' scp:$data/cmvn.scp ark:- ark:- |"
feats_cv="ark:copy-feats scp:$dir/cv.scp ark:- | apply-cmvn --norm-vars='true' scp:$data_cv/cmvn.scp ark:- ark:- |"
# optionally add per-speaker CMVN,
if [ ! -z "$cmvn_opts" ]; then
  echo "Will use CMVN statistics : $data/cmvn_0_d_a.utt.scp, $data_cv/cmvn_0_d_a.utt.scp"
  [ ! -r $data/cmvn_0_d_a.utt.scp ] && echo "Missing $data/cmvn_0_d_a.utt.scp" && exit 1;
  [ ! -r $data_cv/cmvn_0_d_a.utt.scp ] && echo "Missing $data_cv/cmvn_0_d_a.utt.scp" && exit 1;
  feats_tr="$feats_tr apply-cmvn --norm-vars='true' $cmvn_opts scp:$data/cmvn_0_d_a.utt.scp ark:- ark:- |"
  feats_cv="$feats_cv apply-cmvn --norm-vars='true' $cmvn_opts scp:$data_cv/cmvn_0_d_a.utt.scp ark:- ark:- |"
else
