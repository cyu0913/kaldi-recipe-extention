...
cp -rf $srcdata $data; rm $data/{feats,cmvn}.scp 2>/dev/null
...
$cmd JOB=1:1 $logdir/nnet-concat.JOB.log \
...
$cmd JOB=1:1 $logdir/nnet-info.JOB.log \
...
feats="ark,s,cs:copy-feats scp:$sdata/JOB/feats.scp ark:- | apply-cmvn --norm-vars='true' scp:$srcdata/cmvn.scp ark:- ark:-| "
# apply-cmvn (optional),
...
$cmd JOB=1:$nj $logdir/make_bnfeats.JOB.log \
  ...
  ark,scp,t:$bnfeadir/raw_bnfea_$name.JOB.ark,$bnfeadir/raw_bnfea_$name.JOB.scp \
  || exit 1;
