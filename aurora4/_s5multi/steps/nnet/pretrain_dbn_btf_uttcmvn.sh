...
if [ "$copy_feats" == "true" ]; then
  ...
  echo $hname
  mkdir -p /export/$hname/kaldi_tmp
  tmpdir=$(mktemp -d /export/$hname/kaldi_tmp/tmp_XXXX); mv $dir/train.scp{,_non_local}
...
# read the features
feats="ark:copy-feats scp:$dir/train.scp ark:- | apply-cmvn --norm-vars='true' scp:$data/cmvn.scp ark:- ark:-|"
...
for depth in $(seq 1 $nn_depth); do
  ...
  if [ "$depth" == "1" ]; then
    ...
    rbm-train-cd1-frmshuff --learn-rate=$rbm_lrate_low --l2-penalty=$rbm_l2penalty \
      ...
      echo "Initializing '$RBM.init'"
      echo "<NnetProto>
      <Rbm> <InputDim> $num_hid <OutputDim> $btf_dim <VisibleType> bern <HiddenType> bern <ParamStddev> $param_stddev <VisibleBiasCmvnFilename> $dir/$depth.cmvn
      </NnetProto>
      " > $RBM.proto
      nnet-initialize $RBM.proto $RBM.init 2>$dir/log/nnet-initialize.$depth.log || exit 1
      #pre-train
